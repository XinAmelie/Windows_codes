...
def excute_sql(sql):
    ...
    db = MysqlHelp(dbinfo['host'],dbinfo['user'],dbinfo['passwd'],dbinfo['database'],dbinfo['port'])
    db.insert_delete_update(sql)
    return db

def select_sql(sql):
    dbinfo1 = read_yaml(ST.CASEDATA)['DBinfo']
    db = MysqlHelp(dbinfo1['host'], dbinfo1['user'], dbinfo1['passwd'],dbinfo1['port'],dbinfo1['database'])
...
if __name__ == '__main__':
    ...
    # excute_sql(sql)
    sql = "SELECT * FROM hruc.uc_chat_sta sta where sta.dateline = 1632412800 ORDER BY  dateline DESC"
    a = select_sql(sql)
    print(a)
